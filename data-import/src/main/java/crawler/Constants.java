package crawler;

public class Constants {

    public static final String ES_HOST = "localhost";
    public static final int ES_PORT = 9200;
    public static final String SCHEME = "http";

    public static final String USERS_INDEX = "users";
    public static final String USERS_TYPE = "user";

    public static final String TRACK_VECTORS_INDEX = "trackvectors";
    public static final String TRACK_VECTORS_TYPE = "tvec";

    public static String NORMALIZED_TRACK_VECTORS_INDEX = "normalizedtrackvectors";
    public static String NORMALIZED_TRACK_VECTORS_TYPE = "normlTrackVector";

    public static final String USERS_FILE = "users-small.json";
    public static final String APIKey = "685a323d182636518e80a296f620c8a2";

    public static final int num_users = 12;
    public static final int num_tracks = 10;

}
